...
export default function EssenScreen() {
  ...
  const [seg, setSeg] = useState<'tracker' | 'plan'>('tracker');
  ...
  return (
    <View style={styles.container}>
      ...
      <Segmented
        options={[{ key: 'tracker', label: 'Tracker' }, { key: 'plan', label: 'Ernährungsplan' }]}
        value={seg}
        onChange={(k) => setSeg(k as 'tracker' | 'plan')}
        c={c}
      />
      <View style={{ flex: 1, marginTop: 14 }}>
        {seg === 'tracker' ? <FoodTrackerScreen embedded /> : <NutritionScreen embedded />}
      </View>
    </View>
  );
}
